    ...
    def fetch_page(self, page_num: int) -> Optional[str]:
        ...
        try:
            if self.auto_cookie:
                ...
                print(f"🔄 Fetching page {page_num} (auto-cookie mode)...")
                
                # Try POST first (BharatDiscovery prefers POST for content pages)
                response = self.scraper.post(url, timeout=60)
            ...
            if response.status_code == 200:
                ...
                if "Checking your browser" in response.text or "cf-spinner" in response.text:
                    print(f"⚠️ Cloudflare challenge detected for page {page_num}, retrying...")
                    time.sleep(5)  # Wait for challenge to complete
                    if self.auto_cookie:
                        response = self.scraper.post(url, timeout=60)  # Use POST on retry
